...
public class IcsService extends IntentService {
    ...
    private boolean invokeCalendarUI(final Cursor eventCursor, final Bundle extras) {
        ...
        if (0 == startMillis) {
            ...
            if (showEventInfoFullScreen) {
                //BEGIN Motorola, nmr874, 10/09/12, IKMAINJB-127, AOSP BUG / commenting to retain the previous activities for the use case mentioned in the CR.
                //showUI.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
    ...
    private static boolean validateEventData(CalendarEvent calEvent) {
        boolean hasDtstart = calEvent.dtStart != -1L;
        boolean hasDtend = calEvent.dtEnd != -1L;
        boolean hasDuration = !TextUtils.isEmpty(calEvent.duration);

        // Maybe we can try to check the validitiy of RRULE too

        if (!hasDtstart) {
            Log.e(TAG, "vCal event doesn't have valid start time");
            return false;
        }

        if (!hasDuration && !hasDtend) {
            Log.e(TAG, "vCal event doesn't have valid end time");
            return false;
        }
    ...
    private void viewIcs(Intent intent) {
        ...
        if (!validateEventData(vcalEvent)) {
            showToast(this, R.string.invalid_event_time);
            return;
        }
