     ...
     */(  // The name of the shared preferences file. This name must be maintained for historical
        ...
        private inner class AsyncTZHandler(cr: ContentResolver?) : AsyncQueryHandler(cr) {
            protected override fun onQueryComplete(token: Int, cookie: Any?, cursor: Cursor?) {
                synchronized(mTZCallbacks) {
                    ...
                    cursor.close()
                    if (writePrefs) {
                        val prefs: SharedPreferences = getSharedPreferences(cookie as Context, mPrefsName)
                        // Write the prefs
        ...
        fun setTimeZone(context: Context, timeZone: String) {
            ...
            if (updatePrefs) {
                ...
                values.put(CalendarCache.VALUE, if (mUseHomeTZ) CalendarCache.TIMEZONE_TYPE_HOME else CalendarCache.TIMEZONE_TYPE_AUTO)
        ...
        companion object {
            ...
            private val TIMEZONE_INSTANCES_ARGS = arrayOf<String>(CalendarCache.KEY_TIMEZONE_INSTANCES)
