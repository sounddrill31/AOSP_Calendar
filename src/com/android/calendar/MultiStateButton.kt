 ...
 *
 * **XML attributes**
 * See [ MultiStateButton Attributes][R.styleable.MultiStateButton], [Button][android.R.styleable.Button], [TextView Attributes][android.R.styleable.TextView], [ ][android.R.styleable.View]
 *
 */
class MultiStateButton(context: Context?, attrs: AttributeSet?, defStyle: Int) : Button(context, attrs, defStyle) {
    ...
    fun transitionState() {
        state = (state + 1) % mMaxStates
        setButtonDrawable(mButtonResources[state])
    }
    ...
    fun setState(state: Int): Boolean {
        ...
        this.state = state
        setButtonDrawable(mButtonResources[this.state])
        return true
    }
