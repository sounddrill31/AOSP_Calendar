...
object Utils {
  ...
  private val TAG = "CalUtils"

  // Set to 0 until we have UI to perform undo
  val UNDO_DELAY: Long = 0

  // For recurring events which instances of the series are being modified
  val MODIFY_UNINITIALIZED = 0
  val MODIFY_SELECTED = 1
  val MODIFY_ALL_FOLLOWING = 2
  val MODIFY_ALL = 3

  // When the edit event view finishes it passes back the appropriate exit
  // code.
  val DONE_REVERT = 1 shl 0
  val DONE_SAVE = 1 shl 1
  val DONE_DELETE = 1 shl 2

  // And should re run with DONE_EXIT if it should also leave the view, just
  // exiting is identical to reverting
  val DONE_EXIT = 1 shl 0
  val OPEN_EMAIL_MARKER = " <"
  val CLOSE_EMAIL_MARKER = ">"
  val INTENT_KEY_DETAIL_VIEW = "DETAIL_VIEW"
  val INTENT_KEY_VIEW_TYPE = "VIEW"
  val INTENT_VALUE_VIEW_TYPE_DAY = "DAY"
  val INTENT_KEY_HOME = "KEY_HOME"
  val MONDAY_BEFORE_JULIAN_EPOCH = Time.EPOCH_JULIAN_DAY - 3
  val DECLINED_EVENT_ALPHA = 0x66
  val DECLINED_EVENT_TEXT_ALPHA = 0xC0
  private val SATURATION_ADJUST = 1.3f
  private val INTENSITY_ADJUST = 0.8f

  // Defines used by the DNA generation code
  val DAY_IN_MINUTES = 60 * 24
  val WEEK_IN_MINUTES = DAY_IN_MINUTES * 7
  ...
  // was created.
  val SHARED_PREFS_NAME = "com.android.calendar_preferences"
  val KEY_QUICK_RESPONSES = "preferences_quick_responses"
  val KEY_ALERTS_VIBRATE_WHEN = "preferences_alerts_vibrateWhen"
  val APPWIDGET_DATA_TYPE = "vnd.android.data/update"
  val MACHINE_GENERATED_ADDRESS = "calendar.google.com"
  private val mTZUtils: TimeZoneUtils = TimeZoneUtils(SHARED_PREFS_NAME)
  var allowWeekForDetailView = false
  internal var tardis: Long = 0
    private set
  private var sVersion: String? = null
  private val mWildcardPattern = Pattern.compile("^.*$")
   ...
   */
  private val COORD_DEGREES_LATITUDE = ("([-+NnSs]" + "(\\s)*)?"
    + "[1-9]?[0-9](\u00B0)" + "(\\s)*"
    + "([1-5]?[0-9]\')?" + "(\\s)*"
    + "([1-5]?[0-9]" + "(\\.[0-9]+)?\")?"
    + "((\\s)*" + "[NnSs])?")
  private val COORD_DEGREES_LONGITUDE = ("([-+EeWw]" + "(\\s)*)?"
    + "(1)?[0-9]?[0-9](\u00B0)" + "(\\s)*"
    + "([1-5]?[0-9]\')?" + "(\\s)*"
    + "([1-5]?[0-9]" + "(\\.[0-9]+)?\")?"
    + "((\\s)*" + "[EeWw])?")
  private val COORD_DEGREES_PATTERN = (COORD_DEGREES_LATITUDE
    + "(\\s)*" + "," + "(\\s)*"
    + COORD_DEGREES_LONGITUDE)
  private val COORD_DECIMAL_LATITUDE = ("[+-]?"
    + "[1-9]?[0-9]" + "(\\.[0-9]+)"
    + "(\u00B0)?")
  private val COORD_DECIMAL_LONGITUDE = ("[+-]?"
    + "(1)?[0-9]?[0-9]" + "(\\.[0-9]+)"
    + "(\u00B0)?")
  private val COORD_DECIMAL_PATTERN = (COORD_DECIMAL_LATITUDE
    + "(\\s)*" + "," + "(\\s)*"
    + COORD_DECIMAL_LONGITUDE)
  private val COORD_PATTERN = Pattern.compile(COORD_DEGREES_PATTERN + "|" + COORD_DECIMAL_PATTERN)
  private val NANP_ALLOWED_SYMBOLS = "()+-*#."
  private val NANP_MIN_DIGITS = 7
  private val NANP_MAX_DIGITS = 11
   ...
   */
  val isKeyLimePieOrLater: Boolean
    get() = Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT

  fun getViewTypeFromIntentAndSharedPref(activity: Activity): Int {
    val intent = activity.intent
    val extras = intent.extras
    val prefs: SharedPreferences = GeneralPreferences.getSharedPreferences(activity)
    if (TextUtils.equals(intent.action, Intent.ACTION_EDIT)) {
    ...
    if (extras != null) {
      ...
      } else if (INTENT_VALUE_VIEW_TYPE_DAY == extras.getString(INTENT_KEY_VIEW_TYPE)) {
   ...
   */
  fun getWidgetUpdateAction(context: Context): String {
    return context.packageName + ".APPWIDGET_UPDATE"
  }

  /**
   * Gets the intent action for telling the widget to update.
   */
  fun getWidgetScheduledUpdateAction(context: Context): String {
    return context.packageName + ".APPWIDGET_SCHEDULED_UPDATE"
  }
   ...
   */
  fun formatDateRange(
    context: Context?, startMillis: Long, endMillis: Long, flags: Int
  ): String {
    return mTZUtils.formatDateRange(context, startMillis, endMillis, flags)
  }

  fun getDefaultVibrate(context: Context, prefs: SharedPreferences): Boolean {
    val vibrate: Boolean
    if (prefs.contains(KEY_ALERTS_VIBRATE_WHEN)) {
      ...
      // always -> on
      val vibrateWhen = prefs.getString(KEY_ALERTS_VIBRATE_WHEN, null)
      vibrate = vibrateWhen != null && vibrateWhen == context
        .getString(R.string.prefDefault_alerts_vibrate_true)
      prefs.edit().remove(KEY_ALERTS_VIBRATE_WHEN).commit()
      Log.d(
        TAG, "Migrating KEY_ALERTS_VIBRATE_WHEN(" + vibrateWhen
  ...
  fun getSharedPreference(
    ...
    if (ss != null) {
      ...
      return ss.toArray<String>(strings)
    }
    return defaultValue
  }
  ...
  fun setSharedPreference(context: Context?, key: String?, values: Array<String>) {
    val prefs: SharedPreferences = GeneralPreferences.getSharedPreferences(context)
   ...
   */
  fun setDefaultView(context: Context?, viewId: Int) {
    val prefs: SharedPreferences = GeneralPreferences.getSharedPreferences(context)
    val editor = prefs.edit()
  ...
  fun matrixCursorFromCursor(cursor: Cursor?): MatrixCursor? {
    if (cursor == null) {
      return null
    }
   ...
   */
  fun compareCursors(c1: Cursor?, c2: Cursor?): Boolean {
    if (c1 == null || c2 == null) {
      return false
    }
    val numColumns = c1.columnCount
    if (numColumns != c2.columnCount) {
      return false
    }
   ...
   */
  fun timeFromIntentInMillis(intent: Intent): Long {
    // If the time was specified, then use that. Otherwise, use the current
    // time.
    val data = intent.data
    var millis = intent.getLongExtra(CalendarContract.EXTRA_EVENT_BEGIN_TIME, -1)
    if (millis == -1L && data != null && data.isHierarchical) {
      val path = data.pathSegments
      if (path.size == 2 && path[0] == "time") {
        try {
          millis = java.lang.Long.valueOf(data.lastPathSegment)
  ...
  fun formatMonthYear(context: Context?, time: Time): String {
    val flags = (DateUtils.FORMAT_SHOW_DATE or DateUtils.FORMAT_NO_MONTH_DAY
      or DateUtils.FORMAT_SHOW_YEAR)
    val millis = time.toMillis(true)
   ...
   */
  fun join(things: List<*>, delim: String?): String {
    val builder = StringBuilder()
    var first = true
    for (thing: Any in things) {
      if (first) {
        first = false
      } else {
   ...
   */
  fun getJulianMondayFromWeeksSinceEpoch(week: Int): Int {
  ...
  fun getFirstDayOfWeek(context: Context?): Int {
    ...
    } else return if (startDay == Calendar.MONDAY) {
      Time.MONDAY
    } else {
      Time.SUNDAY
    }
   ...
   */
  fun getFirstDayOfWeekAsCalendar(context: Context?): Int {
   ...
   */
  fun getShowWeekNumber(context: Context?): Boolean {
    val prefs: SharedPreferences = GeneralPreferences.getSharedPreferences(context)
   ...
   */
  fun getHideDeclinedEvents(context: Context?): Boolean {
    val prefs: SharedPreferences = GeneralPreferences.getSharedPreferences(context)
    return prefs.getBoolean(GeneralPreferences.KEY_HIDE_DECLINED, false)
  }

  fun getDaysPerWeek(context: Context?): Int {
    val prefs: SharedPreferences = GeneralPreferences.getSharedPreferences(context)
   ...
   */
  fun isSaturday(column: Int, firstDayOfWeek: Int): Boolean {
   ...
   */
  fun isSunday(column: Int, firstDayOfWeek: Int): Boolean {
   ...
   */
  fun convertAlldayUtcToLocal(recycle: Time?, utcTime: Long, tz: String?): Long {
    var recycle = recycle
    if (recycle == null) {
      recycle = Time()
    }
  ...
  fun convertAlldayLocalToUTC(recycle: Time?, localTime: Long, tz: String?): Long {
    var recycle = recycle
    if (recycle == null) {
      recycle = Time()
    }
   ...
   * @param tz The time zone to convert this time to.
   */
  fun getNextMidnight(recycle: Time?, theTime: Long, tz: String?): Long {
    var recycle = recycle
    if (recycle == null) {
      recycle = Time()
    }
  ...
  fun getConfigBool(c: Context, key: Int): Boolean {
    return c.resources.getBoolean(key)
  }
  ...
  fun getDeclinedColorFromColor(color: Int): Int {
    val bg = -0x1
  ...
  fun trySyncAndDisableUpgradeReceiver(context: Context) {
    val pm = context.packageManager
    val upgradeComponent = ComponentName(context, UpgradeReceiver::class.java)
    ...
    ContentResolver.requestSync(
      null /* no account */,
      Calendars.CONTENT_URI.authority,
      extras
    )
   ...
   * @return
   */
  fun createDNAStrands(
    firstJulianDay: Int,
    events: ArrayList<Event>?, top: Int, bottom: Int, minPixels: Int, dayXs: IntArray?,
    context: Context?
    ...
    if (events == null || events.isEmpty() || dayXs == null || dayXs.size < 1 || bottom - top < 8 || minPixels < 0) {
      Log.e(
        TAG,
    ...
    for (currEvent: Event in events) {
      ...
      var startMinute: Int = (event.startDay - firstJulianDay) * DAY_IN_MINUTES + event.startTime
      var endMinute = Math.max(
      ...
      if (startMinute < lastSegment.endMinute) {
        ...
        while (--i >= 0 && endMinute < segments[i].startMinute);
        var currSegment: DNASegment
        // for each segment this event intersects with
        while (i >= 0 && startMinute <= (segments[i].also { currSegment = it }).endMinute) {
          ...
          if (i + 1 < segments.size) {
            ...
            if ((rhs.color == CONFLICT_COLOR) && (currSegment.day == rhs.day
              ...
              strands[currSegment.color]!!.count--
  ...
  private fun addAllDayToStrands(
    event: Event, strands: HashMap<Int, DNAStrand>,
    firstJulianDay: Int, numDays: Int
  ) {
    ...
    val end = Math.min(event.endDay - firstJulianDay, numDays - 1)
    for (i in Math.max(event.startDay - firstJulianDay, 0)..end) {
  ...
  private fun weaveDNAStrands(
    segments: LinkedList<DNASegment>, firstJulianDay: Int,
    strands: HashMap<Int, DNAStrand>, top: Int, bottom: Int, dayXs: IntArray
  ) {
    ...
    for (segment: DNASegment in segments) {
      // Add the points to the strand of that color
      val strand = strands[segment.color]
      ...
      if (DEBUG) {
        Log.d(
          TAG, ("Adding " + Integer.toHexString(segment.color) + " at x,y0,y1: " + x
  ...
  private fun addNewSegment(
    segments: LinkedList<DNASegment>, event: Event,
    strands: HashMap<Int, DNAStrand>, firstJulianDay: Int, minStart: Int, minMinutes: Int
  ) {
    ...
    // day
    val minEnd = Math.min(segment.startMinute + minMinutes, endOfDay)
  ...
  private fun getOrCreateStrand(strands: HashMap<Int, DNAStrand>, color: Int): DNAStrand {
    var strand = strands[color]
    if (strand == null) {
      strand = DNAStrand()
      strand.color = color
      strand.count = 0
      strands[strand.color] = strand
    }
    return strand
  }
   ...
   * @param context
   */
  fun returnToCalendarHome(context: Context) {
    val launchIntent = Intent(context, AllInOneActivity::class.java)
    launchIntent.action = Intent.ACTION_DEFAULT
  ...
  fun getDayOfWeekString(
    julianDay: Int, todayJulianDay: Int, millis: Long,
    context: Context
  ): String {
    getTimeZone(context, null)
    val flags = DateUtils.FORMAT_SHOW_WEEKDAY
    var dayViewText: String
    if (julianDay == todayJulianDay) {
      dayViewText = context.getString(
  ...
  // do run the runnable
  fun setMidnightUpdater(h: Handler?, r: Runnable?, timezone: String?) {
    ...
    time.set(now)
    val runInMillis = ((((24 * 3600) - (time.hour * 3600) - (time.minute * 60) -
  ...
  fun resetMidnightUpdater(h: Handler?, r: Runnable?) {
    if (h == null || r == null) {
      return
    }
    h.removeCallbacks(r)
   ...
   */
  fun getDisplayedDatetime(
    startMillis: Long, endMillis: Long, currentMillis: Long,
    localTimezone: String?, allDay: Boolean, context: Context
  ): String? {
    ...
    if (allDay) {
      ...
      if (singleDayEvent(startMillis, endMillis, currentTime.gmtoff)) {
        ...
        // If possible, use "Today" or "Tomorrow" instead of a full date string.
        val todayOrTomorrow = isTodayOrTomorrow(
          context.resources, startMillis,
          currentMillis, currentTime.gmtoff
        )
        if (TODAY == todayOrTomorrow) {
          // Example: "Today at 1:00pm - 2:00 pm"
          datetimeString = resources.getString(
        ...
        datetimeString = formatDateRange(
          context, startMillis, endMillis,
          flagsDatetime
        )
      }
   ...
   */
  fun getDisplayedTimezone(
    startMillis: Long, localTimezone: String?,
  ...
  private fun singleDayEvent(startMillis: Long, endMillis: Long, localGmtOffset: Long): Boolean {
    ...
    // time end-1.
    val startDay = Time.getJulianDay(startMillis, localGmtOffset)
    val endDay = Time.getJulianDay(endMillis - 1, localGmtOffset)
    return startDay == endDay
  }

  // Using int constants as a return value instead of an enum to minimize resources.
  private val TODAY = 1
  private val TOMORROW = 2
  private val NONE = 0
  ...
  private fun isTodayOrTomorrow(
    r: Resources, dayMillis: Long,
    currentMillis: Long, localGmtOffset: Long
  ): Int {
    val startDay = Time.getJulianDay(dayMillis, localGmtOffset)
    val currentDay = Time.getJulianDay(currentMillis, localGmtOffset)
    val days = startDay - currentDay
    if (days == 1) {
      return TOMORROW
    } else return if (days == 0) {
      TODAY
    } else {
      NONE
    }
  ...
  fun getQuickResponses(context: Context): Array<String> {
    ...
    if (s == null) {
      s = context.resources.getStringArray(R.array.quick_response_defaults)
    }
    return s
  }
  ...
  fun getVersionCode(context: Context): String? {
    if (sVersion == null) {
      try {
        sVersion = context.packageManager.getPackageInfo(
          context.packageName, 0
        ).versionName
      } catch (e: PackageManager.NameNotFoundException) {
        // Can't find version; just leave it blank.
        Log.e(TAG, "Error finding package " + context.applicationInfo.packageName)
      }
    }
    return sVersion
  }
  ...
  class DNAStrand() {
    ...
    var allDays // color for the allday, 0 means no event
      : IntArray?
    var position = 0
    var color = 0
    var count = 0
  ...
  private class DNASegment() {
    var startMinute // in minutes since the start of the week
      = 0
    var endMinute = 0
    var color // Calendar color or black for conflicts
      = 0
    var day // quick reference to the day this segment is on
      = 0
  }
}
