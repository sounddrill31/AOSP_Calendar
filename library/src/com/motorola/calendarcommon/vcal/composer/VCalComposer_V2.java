...
public class VCalComposer_V2 implements VCalComposer {
    ...
    */
    ...
    public byte[] composeVCal() {
        ...
        // VTIMEZONE
        if (!TextUtils.isEmpty(mCe.tzId)) {
            if (DEVELOPMENT) {
                Log.v(TAG, "Append timezone :" + mCe.tzId);
            }
            TimeZone jTz = TimeZone.getTimeZone(mCe.tzId);
    ...
    */
    ...
    private void appendEventBody() {
        ...
        if (TextUtils.isEmpty(mCe.tzId)) {
            ...
            appendDateTime(ICalNames.PROP_START_DATETIME, mCe.tzId, mAsTimeZone.getStandardName(), mCe.dtStart);
            if (TextUtils.isEmpty(mCe.rrule)) {
                appendDateTime(ICalNames.PROP_END_DATETIME, mCe.tzId, mAsTimeZone.getStandardName(), mCe.dtEnd);
            }
        }
