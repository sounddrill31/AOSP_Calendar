 ...
 * 14-Aug-2012    a21263       Initial version
 **********************************************************
 */
...
public class VCalUtils {
    ...
    public int exportEventToSDCard(final Context context, long eventId, String dirName,
            String fileName) {
        return exportEventToSDCard(context, eventId, dirName, fileName, VCalParser.VCAL2);
    }
    ...
    */
    private static ContentProviderOperation appendEventsBuilder(final int calendarId,
        ...
        if (calEvent.organizer != null && !TextUtils.isEmpty(calEvent.organizer.email)) {
            eventValues.put(Events.ORGANIZER, calEvent.organizer.email);
        }
        ...
        if (calEvent.dtEnd != CalendarEvent.INVALID_MIILISEC)
            eventValues.put(Events.DTEND, calEvent.dtEnd); // dtend
        if (calEvent.dtStart != CalendarEvent.INVALID_MIILISEC)
        ...
        if(!TextUtils.isEmpty(calEvent.tzId)){
            if (DEVELOPMENT) {
                Log.v(TAG,"time zone is not null "+calEvent.tzId);
            }
            eventValues.put(Events.EVENT_TIMEZONE,calEvent.tzId);
        }
    ...
    */
    private CalendarEvent getCalendarEvent(Cursor c, ContentResolver resolver) {
        ...
        if (index != -1) {
            if (c.isNull(index))
            {
                event.dtStart = CalendarEvent.INVALID_MIILISEC;
            }
            else
            {
        ...
        if (index != -1) {
            if (c.isNull(index))
            {
                event.dtEnd = CalendarEvent.INVALID_MIILISEC;
            }
            else
            {
    ...
    */
    ...
    private static Uri getScratchCalendarByName(Context context, String name, String displayName) {
        ...
        try {
            ...
            if (c != null && c.moveToFirst()) {
                ...
                values.put(Calendars.CALENDAR_COLOR, Integer.valueOf(0xff0099CC));
    ...
    private static String generateFileName(byte[] content) {
        CRC32 crc = new CRC32();
        crc.update(content);
        return String.valueOf(crc.getValue()) + ".ics";
    }
     ...
     */
    private static Uri createEventInDb(final ContentResolver cr, final Cursor calCursor,
            final CalendarEvent calEvent) {
